...
export async function GET(request) {
  try {
    ...
    const path = businessId ? `/api/businesses/${businessId}` : '/api/businesses';
    const data = await backendRequest(path, {
      apiKey: null,
      cacheMode: 'force-cache',
      revalidate: 60,
      tags: businessId ? [`business:${businessId}`] : ['admin:clients'],
      headers: {
...
export async function POST(request) {
  try {
    ...
    const session = await getAdminSession();
    const data = await backendRequest('/api/businesses', {
      method: 'POST',
      body: payload,
      headers: {
        Authorization: `Bearer ${session.accessToken}`,
